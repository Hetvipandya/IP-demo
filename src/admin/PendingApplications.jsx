...
const handleAssignExecutive = async (applicationId, executiveId) => {
  ...
  const handleViewDetails = async (id) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok && data) setSelectedApp(data);
    } catch (err) {
  ...
  <select
    ...
    onChange={(e) => {
      if (e.target.value) {
        handleAssignExecutive(app._id, e.target.value);
      }
    }}
    className="h-11 w-[170px] rounded-full border border-[#dbe6ff] bg-white px-4 text-xs font-bold text-[#121a43] outline-none transition focus:border-[#1f2f86]"
  >
  ...
  <button
    ...
    className="flex h-11 whitespace-nowrap items-center justify-center gap-2 rounded-full bg-[#1f2f86] px-5 text-xs font-black text-white shadow-lg shadow-[#1f2f86]/20 transition-all hover:bg-[#e6362e] disabled:cursor-not-allowed disabled:opacity-50"
  >
    <Eye size={14} />
    {loadingApplicationId === app._id ? "Loading..." : "View Details"}
  </button>
</div>
              </div>
            ))
