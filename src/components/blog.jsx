      ...
      {blogPosts.map((post) => (
        <div key={post.id} className="bg-white group border-b border-gray-100 pb-16 last:border-0">
          ...
          <Link to={routeMap[post.id]}>
            <h2 className="text-[32px] md:text-[40px] font-bold mb-5 text-[#111] hover:text-blue-600 transition">
              {post.title}
            </h2>
          </Link>

          <p className="text-[#555] text-[16px] md:text-[17px] leading-[1.8] mb-8">
            {post.description}...
          </p>

          <Link to={routeMap[post.id]}>
            <button className="bg-[#f3c669] hover:bg-red-500 hover:text-white px-8 py-3.5 rounded-md text-[14px] font-bold transition">
              Learn More
            </button>
          </Link>
        </div>
 ...
 <aside className="lg:w-[32%] space-y-8">
  ...
  <div className="bg-[#F8F9FA] p-8 rounded-md">
    <div className="mb-6">
      ...
      <div className="w-12 h-1 bg-[#E8021E]"></div>
    </div>
    ...
    <div className="relative">
      <input
        type="text"
        placeholder="Search..."
        className="w-full p-4 border border-gray-200 rounded-md focus:outline-none focus:border-[#213591] text-gray-600 transition"
      />
    </div>
  </div>
      ...
      {sidebarPosts.map((post) => (
        ...
        >
          ...
          <div className="flex flex-col">
            ...
            <h4 className="text-[16px] font-bold text-[#222222] leading-snug transition-colors group-hover:text-[#E8021E] line-clamp-2">
              {post.title}
            </h4>
          </div>
        </div>
